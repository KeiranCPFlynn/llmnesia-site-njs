...
import { CHROME_WEB_STORE_URL, EDGE_ADDONS_URL } from './site.js';
import { PLATFORM_COUNT } from './platforms.js';
...
function taggedInstallUrl(storeUrl, browser) {
  const url = new URL(storeUrl);
  url.searchParams.set('utm_source', 'email');
  url.searchParams.set('utm_medium', 'install_email');
  url.searchParams.set('utm_campaign', 'mobile_install_link');
  url.searchParams.set('utm_content', browser);
  return url.toString();
}

export function installLinkUrl() {
  return taggedInstallUrl(CHROME_WEB_STORE_URL, 'chrome');
}

export function edgeInstallLinkUrl() {
  return taggedInstallUrl(EDGE_ADDONS_URL, 'edge');
}

function buildText({ chrome, edge }) {
  return [
    'Here are your LLMnesia install links:',
    '',
    `Chrome: ${chrome}`,
    `Microsoft Edge: ${edge}`,
    '',
    'Open the link for your browser on your computer — LLMnesia is a free desktop extension for Chrome and Edge.',
    '',
...
function buildHtml({ chrome, edge }) {
...
<html>
  <body style="margin:0;padding:24px;background:#f6f6f4;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;color:#1a1a1a;">
    <div style="max-width:520px;margin:0 auto;background:#ffffff;border:1px solid #e5e5e1;border-radius:12px;padding:28px;">
      <p style="margin:0 0 20px;font-size:16px;line-height:1.55;">Here are your LLMnesia install links.</p>
      <p style="margin:0 0 24px;">
        <a href="${chrome}" style="display:inline-block;margin:0 8px 8px 0;background:#1a1a1a;color:#ffffff;text-decoration:none;font-size:15px;font-weight:600;padding:12px 20px;border-radius:8px;">Add to Chrome — free</a>
        <a href="${edge}" style="display:inline-block;margin:0 0 8px;background:#1a1a1a;color:#ffffff;text-decoration:none;font-size:15px;font-weight:600;padding:12px 20px;border-radius:8px;">Add to Edge — free</a>
      </p>
      <p style="margin:0 0 20px;font-size:15px;line-height:1.55;">Open the link for your browser on your computer. LLMnesia is a free desktop extension for Chrome and Edge.</p>
      ...
      <p style="margin:0 0 4px;font-size:13px;line-height:1.5;color:#6b6b66;">If a button does not work, paste the matching link into your browser:</p>
      <p style="margin:0 0 6px;font-size:13px;line-height:1.5;word-break:break-all;">Chrome: <a href="${chrome}" style="color:#6b6b66;">${chrome}</a></p>
      <p style="margin:0 0 24px;font-size:13px;line-height:1.5;word-break:break-all;">Edge: <a href="${edge}" style="color:#6b6b66;">${edge}</a></p>
...
export async function sendInstallLinkEmail(email) {
  ...
  const links = {
    chrome: installLinkUrl(),
    edge: edgeInstallLinkUrl()
  };
