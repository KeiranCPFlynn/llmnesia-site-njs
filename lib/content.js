...
// Posts published after the CTR triage pull, so they have no GSC row to be
// bucketed from yet. They target the same action intents as bucket A (search,
// find, export, organize) and get the same kinetic demo treatment. Fold these
// into blog-ctr-triage.csv on the next GSC refresh and drop them from here.
const DEMO_EXTRA_SLUGS = new Set([
  ...
  'how-to-export-character-ai-conversation-history',
  'how-to-organize-character-ai-conversation-history',
...
// Choose the kinetic demo for an entry, or null when the page is neither bucket
// A nor a recent post in DEMO_EXTRA_SLUGS. Pages that match no family cleanly
// default to the multi-platform search demo.
export function getDemoForEntry(entry) {
  if (!entry || !entry.slug) return null;
  if (!loadBucketASlugs().has(entry.slug) && !DEMO_EXTRA_SLUGS.has(entry.slug)) return null;
...
// Only the platforms LLMnesia actually supports get a platform-named CTA — the
// 15 web platforms plus local Claude Code and Codex sessions. Unsupported
// platforms we still publish SEO guides for (Poe and NotebookLM) are
// deliberately omitted, so the CTA never claims LLMnesia can search them —
// those pages fall back to the generic multi-platform copy.
// NOTE: 'claude-code' must precede 'claude' — matching is first-substring-wins,
// and every claude-code-* slug also contains 'claude', so the more specific
// token has to be checked first or these pages would be mislabelled "Claude".
...
//   privacy     — employer/training/monitoring anxiety (visibility, not search)
...
const CTA_FAMILY_RULES = [
  ...
  [
    'privacy',
    /privat|privacy|employer|\bboss\b|train(ing)? on your|stop ai from training|monitor(ing)?|confidential|surveil|safe (to use )?(chatgpt |ai )?for work|workplace/
  ],
...
export function getCtaProps(entry) {
  ...
  return {
    platform,
    family: getCtaFamily(entry),
    slug: entry.slug,
    ...(entry.ctaHeadline ? { headline: entry.ctaHeadline } : {}),
    ...(entry.ctaButtonLabel ? { buttonLabel: entry.ctaButtonLabel } : {})
  };
}
