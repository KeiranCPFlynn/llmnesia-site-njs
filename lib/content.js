...
// be placed after the intro. When the article opens straight into a heading
// (no intro paragraph), falls back to splitting after that first whole section
// instead, so the CTA still lands well before the midpoint. Returns null only
// when the article has no `## ` sections at all.
function splitAfterIntro(content) {
  ...
  const splitIndex = lines.slice(0, headingIndices[0]).join('\n').trim()
    ? headingIndices[0]
    : headingIndices[1];
